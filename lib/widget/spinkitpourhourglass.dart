...
class _SpinKitPouringHourglassState extends State<SpinKitPouringHourglass>
  ...
  void startTimer() {
    timer = Timer.periodic(_controller.duration, (timer) {
      if (mounted)
        return setState(() {
          _incrementCounter();
          widget.vibration ? null : Vibration.vibrate(duration: 300);
        });
    });
  }
    ...
    return Center(
      child: Row(
        children: [
          ...
          Text(
            '$_counter',
            style: TextStyle(color: widget.color, fontSize: widget.size * 0.6),
          ),
        ],
      ),
    );
